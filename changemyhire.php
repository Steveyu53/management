...
$id = $_GET['id'];
$sql="select * from house_hire where id='$id'";
$res=mysql_query($sql);
...
if($_POST['submit']){
    $id = $_POST['id'];
    $result =mysql_query("update house_hire set qy='$qy',address='$address',hx='$hx',wylx='$wylx',min_area='$min_area',max_area='$max_area',jg='$jg',zxcd='$zxcd',do_floor='$do_floor',min_pay='$min_pay',max_pay='$max_pay',head='$head',pb='$pb',tel='$tel',man='$man',email='$email',house_ms='$house_ms',date='$date',bus='$bus',pass='否',zj='$zj' where id=$id");
    if($result){
      echo "<script>alert('修改成功！');window.location.href='my_hire.php';</script>";
    }else{
      echo "<script>alert('修改失败！');window.location.href='changemyhire.php';</script>";
    }
