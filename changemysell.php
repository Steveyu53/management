...
$id = $_GET['id'];
$sql="select * from house_sell where id='$id'";
$res=mysql_query($sql);
...
if($_POST['submit']){
    $id = $_POST['id'];
    $result =mysql_query("update house_sell set head='$head',qy='$qy',house_name='$house_name',address='$address',hx='$hx',wylx='$wylx',jz_area='$jz_area',sy_area='$sy_area',jg='$jg',jg_time='$jg_time',zxcd='$zxcd',do_floor='$do_floor',sum_floor='$sum_floor',pay='$pay',pb='$pb',tel='$tel',man='$man',email='$email',house_ms='$house_ms',date='$date',bus='$bus',pass='否',zj='$zj' where id=$id");
    if($result){
      echo "<script>alert('修改成功！');window.location.href='my_sell.php';</script>";
    }else{
      echo "<script>alert('修改失败！');window.location.href='changemysell.php';</script>";
    }
